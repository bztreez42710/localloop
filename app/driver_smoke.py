...
async def _smoke():
    ...
    try:
        # Prepare only the dedicated demo driver and a disposable smoke-test delivery.
        with db() as con:
        ...
        async with httpx.AsyncClient(base_url=BASE,follow_redirects=False,timeout=25.0) as c:
            ...
            if r.status_code not in (303,307) or r.headers.get('location','')!='/driver/app': raise RuntimeError(f'driver dashboard routing {r.status_code} {r.headers.get("location","")}')
            r=await c.get('/driver/app')
            ...
            if r.status_code not in (303,307) or not r.headers.get('location','').startswith('/driver/verify/status'): raise RuntimeError(f'driver setup POST {r.status_code} {r.headers.get("location","")}')
            ...
            if r.status_code not in (303,307) or r.headers.get('location','')!='/driver/app': raise RuntimeError(f'go online {r.status_code} {r.headers.get("location","")}')
            r=await c.get('/driver/app')
            ...
            if r.status_code not in (303,307) or not r.headers.get('location','').startswith('/driver/app'): raise RuntimeError(f'accept {r.status_code} {r.headers.get("location","")}')
            r=await c.get('/driver/app')
            ...
            if r.status_code not in (303,307) or not r.headers.get('location','').startswith('/driver/app'): raise RuntimeError(f'pickup transition {r.status_code}')
            r=await c.get('/driver/app')
            ...
            if r.status_code not in (303,307) or not r.headers.get('location','').startswith('/driver/app'): raise RuntimeError(f'delivery transition {r.status_code}')
            ...
            if r.status_code not in (303,307) or r.headers.get('location','')!='/driver/app': raise RuntimeError(f'go offline {r.status_code}')
