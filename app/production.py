...
# SQLite/Postgres-safe enough for the current app's DB abstraction.
...
def _is_spokane(address:str)->bool:
    a=(address or '').lower()
    z=re.search(r'\b(\d{5})\b',a)
    if z and not z.group(1).startswith(SPOKANE_ZIP_PREFIX):
        return False
    # For typed addresses, require Spokane unless a 992xx ZIP is supplied.
...
def _distance_estimate(pickup:str,dropoff:str,user_distance:float)->float:
    # Until a production routing provider is configured, keep the user's estimate,
    # bounded to a realistic Spokane-local range.
...
def _safe_image(data:str,max_bytes:int=1_750_000)->str:
    ...
    try:
        ...
        raise HTTPException(400,'Could not read photo proof.')
...
def production_startup():
    with db() as con:
        con.execute('''CREATE TABLE IF NOT EXISTS driver_compliance(
            ...
            background_consent INTEGER DEFAULT 0,
            insurance_company TEXT DEFAULT '',
            insurance_policy_last4 TEXT DEFAULT '',
            insurance_expires TEXT DEFAULT '',
        ...
        _add_column(con,'deliveries','pickup_lat REAL')
        _add_column(con,'deliveries','pickup_lng REAL')
        _add_column(con,'deliveries','dropoff_lat REAL')
        _add_column(con,'deliveries','dropoff_lng REAL')
        ...
        _add_column(con,'shopping_orders','payment_intent_id TEXT DEFAULT \'\'')
...
# Replace selected core routes with safer production-aware versions.
for r in list(app.router.routes):
    p=getattr(r,'path',None); methods=getattr(r,'methods',set()) or set()
    if (p=='/deliveries' and 'POST' in methods) or (p=='/driver/online' and 'POST' in methods) or (p=='/deliveries/{did}/status' and 'POST' in methods):
...
def create_delivery_production(request:Request,pickup:str=Form(...),dropoff:str=Form(...),item_description:str=Form(...),distance_miles:float=Form(1),notes:str=Form('')):
    ...
    if not _is_spokane(pickup) or not _is_spokane(dropoff):
        raise HTTPException(400,'LocalLoop currently serves Spokane, Washington only. Please use Spokane addresses (992xx).')
    miles=_distance_estimate(pickup,dropoff,distance_miles)
    q=quote(miles); t=now()
    with db() as con:
        cur=con.execute('INSERT INTO deliveries(customer_id,business_id,pickup,dropoff,item_description,distance_miles,quoted_cents,platform_fee_cents,driver_pay_cents,notes,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(u['id'],u['id'] if u['role']=='business' else None,pickup.strip(),dropoff.strip(),item_description.strip(),miles,q['customer_total_cents'],q['platform_fee_cents'],q['driver_pay_cents'],notes.strip(),t,t))
        event(con,cur.lastrowid,u['id'],'posted',q)
...
def driver_setup_page(request:Request):
    ...
    return page(request,'driver_setup.html',compliance=c,stripe_ready=bool(STRIPE_SECRET_KEY))

@app.post('/driver/setup')
def driver_setup_save(request:Request,background_consent:int=Form(0),insurance_company:str=Form(''),insurance_policy_last4:str=Form(''),insurance_expires:str=Form('')):
    u=require_user(request)
    if u['role']!='driver': raise HTTPException(403)
    last4=re.sub(r'\D','',insurance_policy_last4)[-4:]
    with db() as con:
        con.execute('INSERT OR IGNORE INTO driver_compliance(user_id,updated_at) VALUES(?,?)',(u['id'],now()))
        con.execute('UPDATE driver_compliance SET background_consent=?,insurance_company=?,insurance_policy_last4=?,insurance_expires=?,updated_at=? WHERE user_id=?',(1 if background_consent else 0,insurance_company.strip(),last4,insurance_expires.strip(),now(),u['id']))
...
def driver_online_production(request:Request,online:int=Form(...)):
    ...
    with db() as con:
        ...
        if online and (not c or c['identity_status']!='approved' or c['background_status']!='approved' or c['insurance_status']!='approved'):
            raise HTTPException(403,'Driver verification is not complete. Finish Driver Setup and wait for approval before going online.')
...
def admin_driver_review(uid:int,request:Request,identity_status:str=Form(...),background_status:str=Form(...),insurance_status:str=Form(...)):
    ...
    with db() as con:
        con.execute('INSERT OR IGNORE INTO driver_compliance(user_id,updated_at) VALUES(?,?)',(uid,now()))
        con.execute('UPDATE driver_compliance SET identity_status=?,background_status=?,insurance_status=?,updated_at=? WHERE user_id=?',(identity_status,background_status,insurance_status,now(),uid))
...
def payment_status(request:Request):
    ...
    return JSONResponse({'stripe_configured':bool(STRIPE_SECRET_KEY),'sms_configured':bool(TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN and TWILIO_FROM),'database_backend':'postgres' if os.environ.get('DATABASE_URL','').startswith('postgres') else 'sqlite'})
