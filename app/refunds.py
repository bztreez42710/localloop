...
def issue_marketplace_refund(oid:int,request:Request,refund_amount_cents:int=Form(0)):
    ...
    if not configured(): raise HTTPException(503,'Stripe is not connected yet, so LocalLoop cannot move refund funds automatically.')
    with db() as con:
        ...
        if not order['payment_provider_ref']: raise HTTPException(400,'This order has no Stripe Checkout reference.')
    ...
    session=stripe('GET',f"/checkout/sessions/{order['payment_provider_ref']}")
    payment_intent=(session.get('payment_intent') or '').strip()
    if not payment_intent: raise HTTPException(400,'Stripe has not attached a payment to this order yet.')
    refund=stripe('POST','/refunds',data={
        'payment_intent':payment_intent,
        'amount':str(amount),
        'metadata[localloop_order]':str(oid),
        'metadata[type]':'marketplace_refund',
    })
    state=(refund.get('status') or 'pending').lower(); ref=refund.get('id','')
    resolved=state=='succeeded'
    with db() as con:
        _ensure_marketplace_order_column(con)
        con.execute('INSERT INTO payment_records(user_id,provider,provider_ref,amount_cents,status,kind,marketplace_order_id,created_at) VALUES(?,?,?,?,?,?,?,?)',(order['buyer_id'],'stripe',ref,-amount,state,'marketplace_refund',oid,now()))
        ...
        if resolved:
            con.execute("UPDATE marketplace_orders SET status='refunded',payment_status='refunded',updated_at=? WHERE id=?",(now(),oid))
