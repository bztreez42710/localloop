...
def mobile_offers(request:Request):
    ...
    with db() as con:
        ...
        offers=[dict(r) for r in con.execute("SELECT id,pickup,dropoff,item_description,distance_miles,driver_pay_cents,status FROM deliveries WHERE status='posted' ORDER BY id DESC LIMIT 40").fetchall()]
        active=[dict(r) for r in con.execute("SELECT id,pickup,dropoff,item_description,distance_miles,driver_pay_cents,status,handoff_required FROM deliveries WHERE driver_id=? AND status IN ('accepted','picked_up') ORDER BY id DESC",(u['id'],)).fetchall()]
...
def mobile_location(request:Request,latitude:float=Form(...),longitude:float=Form(...)):
    ...
    with db() as con:
        p=con.execute('SELECT online FROM driver_profiles WHERE user_id=?',(u['id'],)).fetchone()
        active=con.execute("SELECT 1 FROM deliveries WHERE driver_id=? AND status IN ('accepted','picked_up') LIMIT 1",(u['id'],)).fetchone()
        if not p or not p['online'] or not active: raise HTTPException(409,'Location sharing is only available while online with an active delivery')
...
def mobile_earnings(request:Request):
    ...
    with db() as con:
        ...
        rows=[dict(r) for r in con.execute("SELECT l.id,l.delivery_id,l.amount_cents,l.note,l.created_at,d.pickup,d.dropoff FROM ledger l LEFT JOIN deliveries d ON d.id=l.delivery_id WHERE l.user_id=? AND l.kind='driver_earning' ORDER BY l.id DESC LIMIT 100",(u['id'],)).fetchall()]
