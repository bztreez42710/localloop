...
def ensure_account(con,email,password,name,role,staff_role=None):
    ...
    if row: uid=row['id'];con.execute('UPDATE users SET password_hash=?,name=?,role=?,verified=1,active=1 WHERE id=?',(hash_password(password),name,role,uid))
    else: cur=con.execute('INSERT INTO users(email,password_hash,name,role,verified,created_at) VALUES(?,?,?,?,1,?)',(email,hash_password(password),name,role,now()));uid=cur.lastrowid
...
def _has_current_legal(con,u):
    try:
        required=[('terms',TERMS_VERSION),('privacy',PRIVACY_VERSION)]+([('driver_agreement',DRIVER_VERSION)] if u['role']=='driver' else [])
        return all(con.execute('SELECT 1 FROM legal_acceptances WHERE user_id=? AND document_type=? AND version=?',(u['id'],d,v)).fetchone() for d,v in required)
    except Exception:return False
def _verified_for_use(con,u):
    if u['role'] not in {'customer','business'}:return True
    try:r=con.execute('SELECT verification_status FROM account_verifications WHERE user_id=?',(u['id'],)).fetchone();return bool(r and r['verification_status']=='verified')
    except Exception:return False
def _next_for_user(con,u):
    ...
    return '/driver/app' if u['role']=='driver' else '/dashboard'
...
def portal_startup():
    with db() as con:
        ...
        ensure_account(con,OWNER_EMAIL,OWNER_PASSWORD,'LocalLoop Owner','admin');ensure_account(con,SAFETY_EMAIL,SAFETY_PASSWORD,'LocalLoop Safety Monitor','admin','safety');ensure_account(con,DEVELOPER_EMAIL,DEVELOPER_PASSWORD,'LocalLoop Developer','admin','developer');ensure_account(con,DEMO_DRIVER_EMAIL,DEMO_DRIVER_PASSWORD,'LocalLoop Demo Driver','driver');ensure_account(con,DEMO_BUSINESS_EMAIL,DEMO_BUSINESS_PASSWORD,'LocalLoop Demo Store','business');ensure_account(con,DEMO_CUSTOMER_EMAIL,DEMO_CUSTOMER_PASSWORD,'LocalLoop Demo Customer','customer')
...
async def readonly_staff_guard(request:Request,call_next):
    if request.method not in {'GET','HEAD','OPTIONS'} and request.url.path not in {'/login','/logout','/legal/acceptance'}:
...
@app.post('/login')
def portal_login(email:str=Form(...),password:str=Form(...),portal:str=Form('any'),next:str=Form('')):
    with db() as con:u=con.execute('SELECT * FROM users WHERE email=?',(email.lower().strip(),)).fetchone();staff=staff_role_for(con,u['id']) if u else None
    if not u or not u['active'] or not verify_password(password,u['password_hash']):return RedirectResponse('/driver/login?error=1',303) if portal=='driver' else RedirectResponse('/login?error=1',303)
    actual=staff or u['role'];expected={'store':'business','owner':'admin'}.get(portal,portal)
    if expected not in {'any',actual}:return RedirectResponse('/driver/login?wrong=1',303) if portal=='driver' else RedirectResponse('/login?wrong=1',303)
    with db() as con:
    ...
    if next.startswith('/') and not next.startswith('//') and next in {'/driver/app','/dashboard'} and next_path not in {'/legal/acceptance','/account/verification'}:next_path=next
    r=RedirectResponse(next_path,303);r.set_cookie('ll_session',sign(str(u['id'])),httponly=True,samesite='lax',secure=COOKIE_SECURE,max_age=2592000);return r
...
from . import marketplace,production,payments,routing,legal,community_marketplace,marketplace_payments,admin_accounts,ux,community_plus,refunds,verification,storage_integration,infrastructure,job_board,security,driver_app,driver_smoke,mobile_api,tracking_live,driver_payouts,driver_telemetry,driver_experience,driver_multistop,driver_history
